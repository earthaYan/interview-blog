...
自由变量指的是在当前函数作用域内未被声明或定义的变量，但会在函数执行时被访问。此时 JS 会沿着作用域链向上层作用域查找这个变量，直到找到或者报错。

## 闭包

函数作为另一个函数的入参或者返回值，且该内部函数访问了外部函数的自由变量，就会形成闭包

危害：

1. 变量常驻内存，得不到释放。不要乱用，可能会导致内存泄漏
2. 内存泄漏通常发生在闭包长期引用了不再需要的大对象或 DOM 元素，但没有释放引用

解决方案：

1. 不要在闭包里保存不必要的大对象或 DOM 引用；
2. 及时解绑事件回调；
3. 现代框架如 React、Vue 会自动管理这些引用
...
handleClick 函数使用了一个闭包来缓存 count 的值。每次点击触发 handleClick，1s 后 setCount 会将 count 值加 1，在这 1s 内，无论点击多少次，count 值都只会加 1。这是因为 setCount 所接收的 count 值是在闭包中被缓存的 count 值，这个值是始终不变的。1s 后 setTimeout 中的 setCount 生效，函数式组件 Counter 重新执行，会生成新的 handleClick 方法，形成新的闭包，此时闭包中缓存的 count 值也就变成了最新的 count 值。再继续点击，又会重复上述循环，每过 1s count 值会加 1。通过**函数式更新**可以解决
