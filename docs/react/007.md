...
**陷阱 1**:
将一个普通对象设置为依赖项：每次 react 重新渲染，函数都会被重新执行，每次都重新创建这个对象，由于每次创建的对象内存地址都不一样，所以浅比较结果永远是 false。

解决方案：

1. 将这个对象移动到组件外部
2. 如果这个对象和组件内部 state 相关，则使用 useMemo 包裹

**陷阱 2**

将函数作为依赖项：同上，每次重新执行都是创建的新函数

解决方案：

1. 用 useCallback 包裹函数
2. 将函数定义在 Hooks 内部

如何避免一些问题：使用 eslint 的 eslint-plugin-react-hooks 插件，启用 rules-of-hooks/exhaustive-deps 这两个规则
